    ...
    <header className="bg-white border-b border-gray-200 text-[#4255FF] shadow-sm p-4 flex items-center justify-between">
      ...
      {isAuthenticated ? (
        <div className="flex items-center gap-3 mr-4">
          ...
          >
            <Link href="/srs" className="flex items-center gap-2">
              <Brain className="h-4 w-4" />
              <span>Review</span>
            </Link>
          </Button>
          <Link href="/account" className="flex items-center gap-3 hover:opacity-80 transition">
            {isLoading ? (
              ...
              <Avatar className="size-12 border border-gray-100">
                ...
                />
                <AvatarFallback>{me ? getInitials(me.name) : 'U'}</AvatarFallback>
              </Avatar>
            )}
          </Link>
